...
function History() {
    ...
    useEffect(() => {
        ...
        const fetchVolunteeredProblems = async () => {
            try {
                const response = await axios.get(url + `fetch-volunteer-history?email=${localStorage.getItem("volunteerLoginEmail")}&status=${statusType}`)
    ...
    return (
        ...
        >
            <div className='bg-white p-2 py-5 flex items-center shadow-md rounded-md justify-between'>
                ...
                <span style={{ width: '24px', marginRight: '1rem' }}></span> {/* Empty space for balance */}
            </div>
                            ...
                            return (
                                <div key={Math.random()} className='flex flex-col gap-3 bg-white p-5 rounded-md shadow-md'>
                                    ...
                                    <div className='flex flex-row items-center justify-between gap-5'>
                                        <Button fullWidth variant='outlined' disableElevation>Contact Victim</Button>
                                        <IconButton onClick={() => {
