...
const VolunteerRegister = () => {
  const [formData, setFormData] = useState({
    ...
    confirmPassword: "",
    phone: "",
    location: [], // Store location as an array [latitude, longitude]
  });
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          setFormData((prev) => ({
            ...prev,
            location: [latitude, longitude], // Set location as an array
          }));
        },
        (error) => {
  ...
  const handleRegister = async () => {
    ...
    try {
      const response = await axios.post(url + "auth/register-volunteer", {
    ...
    >
      <div className="flex h-screen w-screen flex-row">
        <div className="flex flex-col px-10 basis-3/4 text-center bg-[#F7EFF6] gap-6 justify-center p-6 rounded-md">
          ...
          <p>
            Already Registered as a Volunteer? <b className="cursor-pointer" onClick={() => { navigate("/volunteer-login") }}>Login</b>
          </p>
        </div>
